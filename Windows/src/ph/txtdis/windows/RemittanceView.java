...
public class RemittanceView extends ReportView {
	...
	private Button btnNew, btnPost, btnList;
	...
	protected void setHeader() {
		...
		txtBankName = 
				new DataDisplay(grpPartner, "NAME", cust.getName(), 2).getText();
		...
		txtRefId = new DataEntry(grpInvoice, "REF ID", remit.getRefId()).getText();
		txtRemitId = new DataDisplay(grpInvoice, "REMIT ID", remit.getRemitId()).getText();
		txtTotalPayment = new DataEntry(grpInvoice, "AMOUNT", remit.getTotalPayment()).getText();		
	...
	protected void setFooter() {
		...
		txtBalance = new DataDisplay(cmp, "VARIANCE", remit.getBalance()).getText();
	}
	...
	setListener() {
		...
		new DataInput(txtRefId, txtTotalPayment) {
			...
			protected boolean ifHasText() {
				...
				if (refId <= 0) 
					return false;
				remitId = new RemittanceHelper().getRemitId(bankId, date, time, refId); 
				if (remitId != 0) {
					String deposit = "Deposit Slip #";
					try {
						if(time.equals(new Time(DIS.TF.parse("00:00").getTime())))
							deposit = "Check #";
					} catch (ParseException e) {
						e.printStackTrace();
					}
	...
	private void setSeries() {
		final Button btnNew = new TableButton(
				tableItem, rowIdx, 0, report.getModule() + "16").getButton();	
	...
	private void setOrderIdInput() {
		txtOrderId = new TableInput(tableItem, rowIdx, 2, BigDecimal.ZERO).getText();
		...
		new DataInput(txtOrderId, btnPost){
			...
			protected boolean act() {
				...
				BigDecimal payment = new RemittanceHelper().getPayment(series, orderId);
				BigDecimal totalOfThisOrder = actualOfThisOrder.subtract(payment);
				...
				if(shortId != null && shortId.equals("OR")) { 
					if(orderId < 0 && actualOfThisOrder.equals(totalOfThisOrder)) {
						totalOfThisOrder = actualOfThisOrder.abs();
					} else if(orderId < 0 && payment.compareTo(BigDecimal.ZERO) > 0) {
				...
				if (actualOfThisOrder.equals(BigDecimal.ZERO))  {
					...
					CustomerHelper customerHelper = new CustomerHelper(customerId);
					...
					BigDecimal runningPaymentTotal = remit.getRunningPaymentTotal();
					BigDecimal paymentForThisOrder;
					if (runningOrderTotal.compareTo(totalPayment) > 0) {
						paymentForThisOrder = 
								totalPayment.subtract(runningPaymentTotal);
					} else {
					...
					BigDecimal balance = totalPayment.subtract(runningOrderTotal);
					...
					tableItem.setBackground(
							rowIdx % 2 == 0 ? View.white() : View.gray());
