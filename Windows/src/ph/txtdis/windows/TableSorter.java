	...
	void sortTable(final int columnIndex) {
		...
		Comparator<TableItem> numComparator = new Comparator<TableItem>() {
			...
			public int compare(TableItem t1, TableItem t2) {
				...
				double i1 = s1.equals("") ? 0 : Double.parseDouble(s1.replace(",", ""));
				double i2 = s2.equals("") ? 0 : Double.parseDouble(s2.replace(",", ""));
				if (i1 < i2) return updown * 1;
				if (i1 > i2) return updown * -1;
				return 0;
			}
		};
		...
		for(int i = 0; i < tableItems.length; i++) {
			...
			for(int j = 0; j < table.getColumnCount(); j++) {
				...
				if (	headers[j][1].contains("BigDecimal") &&
						!strItem.isEmpty()) {
				...
				{
					tblItem.setForeground(j, View.red());
				} else {
					tblItem.setForeground(j, View.black());
				}
			}
			tblItem.setChecked(tableItems[i].getChecked());
			tableItems[i].dispose();
			tblItem.setBackground(i % 2 == 0 ? View.white() : View.gray());
		}
