...
public class ReceivingList extends RoutedData implements Subheaded {
	private int itemId;

	public ReceivingList(Date[] dates, int itemId, Integer routeId, Integer qcId) {
		type = Type.RECEIVING_LIST;
		this.dates = dates;
		this.itemId = itemId;
	...
	private Object[][] getPerRouteList(Date start, Date end, int itemId, int routeId) {
		return new Query().getTableData(new Object[] {start, end, itemId, routeId}, "" 
				...
				+ "SELECT CAST (row_number() OVER (ORDER BY receiving_id) AS int),\n"
				+ " 	  receiving_id,\n"
				+ " 	  receiving_date,\n"
				+ "		  name,\n" 
				+ "		  qty\n" 
	...
	private Object[][] getTotalList(Date start, Date end, int itemId, int qcId) {
		return new Query().getTableData(new Object[] {start, end, itemId, qcId}, "" 
				+ "SELECT CAST (row_number() OVER (ORDER BY rh.receiving_id) AS int),\n"
				+ " 	  rh.receiving_id,\n"
				+ " 	  rh.receiving_date,\n"
				+ "		  cm.name,\n" 
    ...
    public String getSubheading() {
		String productReceived = Item.getName(itemId);
		if (routeId != null)
			productReceived += "\nback-loaded from " + Route.getName(routeId);
		else 
			productReceived += "\nreturned/purchased";
