...
public class ReceivingHelper {
	...
	public boolean hasId(int rrId) {
		Object o = new SQL().getDatum(rrId, "" + "SELECT rr_id  "
				+ "FROM 	receiving_header " + "WHERE 	rr_id = ? ");
	...
	public Object[][] getReceivedReturnedMaterials(int soId) {
		Object[][] aao = new SQL().getDataArray(soId, ""
				...
				+ "GROUP BY rd.item_id ORDER BY rd.item_id ");
		return aao;
	}

	public Object[][] getReceivedMaterials(int soId) {
		Object[][] aao = new SQL().getDataArray(soId, ""
				+ "SELECT rd.item_id, sum(rd.qty * qp.qty) "
				+ "FROM receiving_detail AS rd "
				+ "INNER JOIN receiving_header AS rh ON rd.rr_id = rh.rr_id "
				+ "INNER JOIN qty_per AS qp "
				+ " ON rd.item_id = qp.item_id AND rd.uom = qp.uom "
				+ "WHERE ref_id = ? "
				+ "GROUP BY rd.item_id ORDER BY rd.item_id ");
		return aao;
	}
