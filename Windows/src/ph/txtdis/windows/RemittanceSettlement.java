...
public class RemittanceSettlement extends Report {

	public RemittanceSettlement(Date[] dates, int routeId) {
		module = "Remittance Settlement";
		...
		headers = new String[][] { 
		        ...
		        { StringUtils.center(DIS.CURRENCY_SIGN + " PAYMENT", 14), "BigDecimal" },
		        { StringUtils.center("REMIT #", 8), "ID" },
		        { StringUtils.center(DIS.CURRENCY_SIGN + " DEPOSIT", 14), "BigDecimal" },
		        { StringUtils.center(DIS.CURRENCY_SIGN + " GAIN(LOSS)", 14), "BigDecimal" } };

		data = new Data().getDataArray(new Object[] { dates[0], dates[1], routeId, DIS.MAIN_CASHIER }, ""
				// @sql:on
				+ "WITH parameter\n" 
				...
				+ "                INNER JOIN remittance_detail AS rd\n" 
				...
				+ "                INNER JOIN deposited_remittance AS dr\n" 
	...
	public BigDecimal getTotalVariance() {
		return data[0][7] == 	null ? BigDecimal.ZERO : (BigDecimal) data[0][7];
	}
}
