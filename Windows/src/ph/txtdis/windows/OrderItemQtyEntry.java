	...
	public OrderItemQtyEntry(
		...
		txtQty.addListener (SWT.DefaultSelection, new Listener () {
			...
			public void handleEvent (Event e) {
				...
				boolean isDisposal = view.getTxtPartnerName().getText().trim().equals("BO DISPOSAL");
				...
				if(isMonetary && qty.multiply(price).compareTo(order.getActual()) != 0) {
				...
				if(isDisposal && !hasEnoughBadStock) {
					clearText("Only " + DIS.BIF.format(badStock) + " available;\nplease adjust quantity");
					return;						
				} else if(!isDisposal && !isMonetary && !hasEnoughGoodStock) {
					clearText("Only " + DIS.BIF.format(goodStock) + " available;\nplease adjust quantity");
					return;
				}
				uom = lineItem.getUom();
				...
				if(uom == new VolumeDiscount().getUom(itemId, date)) {
					subtotal = (price.multiply(qty)).subtract(
							volumeDiscount.multiply(
									qty.divide(perQty, 0, BigDecimal.ROUND_DOWN)));
				} else {
				...
				if(isBO && isSO) {
					...
					if (balance.compareTo(BigDecimal.ZERO) < 0) {
						...
						view.getTxtActual().setText(DIS.LNF.format(order.getActual()));
					}
				}
				...
				discount1 = subtotal.multiply(discountRate1
				...
				discount2 = (subtotal.subtract(discount1)).multiply(discountRate2
						.divide(new BigDecimal(100), BigDecimal.ROUND_HALF_EVEN));
				...
				vat = net.subtract(net.divide(vatRate, BigDecimal.ROUND_HALF_EVEN));
				if(isMonetary) vat = BigDecimal.ZERO;
				order.setTotalVat(order.getTotalVat().add(vat));
				view.getTxtTotalVat().setText(DIS.LNF.format(order.getTotalVat()));
				// show VATable
				BigDecimal totalVatable = order.getTotalVatable().add(net.subtract(vat));
				if(isMonetary) totalVatable = BigDecimal.ZERO;
				order.setTotalVatable(totalVatable);
				view.getTxtTotalVatable().setText(DIS.LNF.format(order.getTotalVatable()));
				// show total
