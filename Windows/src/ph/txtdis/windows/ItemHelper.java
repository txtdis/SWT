...
public class ItemHelper {
	...
	public int getId(String shortId)  {
		Integer o =  (Integer) new SQL().getDatum(shortId, "" + 
				"SELECT id " +
				"FROM 	item_master " +
				"WHERE 	short_id = ? " 
				);
	...
	public int getId(long unspsc)  {
		Integer o =  (Integer) new SQL().getDatum(unspsc, "" +
				"SELECT id " +
				"FROM 	item_master " +
				"WHERE 	unspsc_id = ? " 
				);
	...
	public String getShortId(String name)  {
		return  (String) new SQL().getDatum(name, 
				"SELECT short_id " +
						"FROM 	item_master " +
						"WHERE 	name = ? " 
				);
	...
	public String getFamilyName(int familyId)  {
		return (String) new SQL().getDatum(familyId, "" +
				"SELECT name " +
				"FROM 	item_family " +
				"WHERE 	id = ? " 
				);
	...
	public int getFamilyId (String familyName)  {
		Integer id = (Integer) new SQL().getDatum(familyName, "" +
				"SELECT id " +
				"FROM 	item_family " +
				"WHERE 	name = ? " 
				);
	...
	public String[] getFamilies(int tierId)  {
		Object[] objects = new SQL().getData(tierId, "" +
				"SELECT name " +
				"FROM 	item_family " +
				"WHERE 	tier_id = ? " +
				"ORDER BY name "
	...
	public int getTypeId(String name)  {
		return (Integer) new SQL().getDatum (name, "" +
				"SELECT id " +
				"FROM 	item_type " +
				"WHERE 	name = ? " +
				"");
	...
	public Integer getProductLineId(String productLine) {
		return (Integer) new SQL().getDatum(productLine, "" +
				"SELECT	id " +
				"FROM	item_family " +
				"WHERE	name = ? " +
				"");
	...
	public boolean isNotDiscounted(int id) {
		return (boolean) new SQL().getDatum(id, "" +
	...
	public BigDecimal getAvailableStock(int itemId) {
		Object obj = new SQL().getDatum(itemId, "" +
				"SELECT	good " +
				"FROM	inventory " +
				"WHERE	id = ? " +
				"");
