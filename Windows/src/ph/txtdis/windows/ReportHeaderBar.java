	...
	public ReportHeaderBar(Composite parent, Report report) {
		...
		switch (report.getModule()) {
			...
			case "Invoice/Delivery List":
				...
				if (il.getCategoryId() == null) {
					...
					if (il.getRouteId() != null) 
						string +=  "by " + new RouteHelper().getRoute(il.getRouteId());
					string += "\n";

				} else {
					string += new ItemHelper().getFamilyName(il.getProductLineId());
			...
			case "Outlet List":
				...
				string += " sold by ";
				string += new RouteHelper().getRoute(ol.getRouteId());
				string += "\n";
				dates = ol.getDates();
				break;
			...
			case "Overdue Statement":
				dates = new Date[0];
				string = new CustomerHelper(((OverdueStatement) report)
			...
			case "Receiving Report List":
				...
				if(rl.getRouteId() != null)
					string += "\nback-loaded from " + 
							new RouteHelper().getRoute(rl.getRouteId());
				else
					string += "\nreturned/purchased";
				string += "\n";
				dates = rl.getDates();
			...
			case "Sales Report":
				...
				string = (sr.getMetric().equals("SALES TO TRADE")
						? "Sales to Trade of " : "Productivity for ") + string;
				dates = sr.getDates();
				break;
			case "Stock Take":
